    ...
    var promise = Q.Promise(function (resolve, reject, notify) {
        ...
        function onSuccess() {
            ...
            if (_response.status >= 200 && _response.status < 300) {
                ...
                (reject || function () {
                })(new Error('[Travhaller] API Request to \'' + url + '\' failed with Status code ' + _response.status));
            }
        }

        function onError() {
            (reject || function () {
            })(new Error('Can\'t XHR ' + JSON.stringify(url)));
        }
            ...
            return promise.then(function () {
                (resolve || function () {
                    ...
                    follow: function (linkname) {
                        if (_resource._links[linkname] === undefined) {
                            console.error('[Travhaller] Can\'t follow link \'' + linkname + '\' because it does not exist in current resource.');
                            return this;
                        } else {
